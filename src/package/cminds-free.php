...
if ( !defined( __NAMESPACE__ . '\PLATFORM_VERSION' ) ) {
	define( __NAMESPACE__ . '\PLATFORM_VERSION', '1_0_7' );
}
...
//			add_action( 'cminds-' . $aboutPageKey . '-content-5', array( $this, 'displayUpgradeToProTab' ) );
		...
		function submitUninstallReason() {
			...
			if ( !empty( $filteredReason ) ) {
				...
				$result = wp_mail( $mail , 'Comentarismo Plugin Deactivation Feedback', $message );
		...
		public function showAuthor( $atts = array() ) {
			...
			$authorUrl .= '<a href="' . $currentPlugin->addAffiliateCode( 'https://www.comentarismo.com/' ) . '" target="_blank" class="cminds_poweredbylink">Comentarismo</a> ';
			$authorUrl .= ' <a href="' . $currentPlugin->addAffiliateCode( 'https://api.comentarismo.com/plugin/' ) . '" target="_blank" class="cminds_poweredbylink">WordPress Plugin</a>';
		...
		public function displayUpgradeTutorialTab() {
			...
			?>
			<div>
				<a href="https://admin.comentarismo.com/" class="btn button button-primary" target="_blank">Customer Dashboard</a>
			</div>
			<br/>
			<div>
		...
		public function displayManageProductsTab() {
			...
			?>
			<div>
				<a href="https://www.comentarismo.com/guest-account/" class="btn button button-primary" target="_blank">Open CreativeMinds Customer Dashboard</a>
			</div>
			<br/>
			<div>
		...
		public function displayCreativeMindsTab() {
			...
			?>
			<div>
				<a href="https://www.comentarismo.com/about/" class="btn button button-primary" target="_blank">Open About Us page in a new tab</a>
			</div>
			<br/>
			<div>
				<iframe src="https://www.comentarismo.com/about/?cminds_iframed=1" height="700" style="width: 100%" ></iframe>
			</div>
			<?php
			$content .= ob_get_clean();
			return $content;
		...
		public function displayMembershipTab() {
			...
			<div>
				<a href="https://www.comentarismo.com/store/cm-wordpress-plugins-yearly-membership/" class="btn button button-primary" target="_blank">Open Membership Package page in a new tab</a>
			</div>
			<br/>
			<div>
				<iframe src="https://www.comentarismo.com/store/cm-wordpress-plugins-yearly-membership/?cminds_iframed=1" height="700" style="width: 100%" ></iframe>
			</div>
			<?php
			$content .= ob_get_clean();
			return $content;
		...
		public function displayCMProductTab() {
			...
			<div>
				<a href="https://www.comentarismo.com/store/?cat=Plugin" class="btn button button-primary" target="_blank">Open Product Catalog in a new tab</a>
			...
			<br/>
			<div>
				<iframe src="https://www.comentarismo.com/store/?cminds_iframed=1&showfilter=No&category=<?php echo $currentIframeFilter; ?>" height="700" style="width: 100%" ></iframe>
			</div>
		...
		public function displayCMAddOnsTab() {
			...
			<div>
				<a href="https://www.comentarismo.com/store/?showfilter=No&tags=Tooltip" class="btn button button-primary" target="_blank">Open Add-Ons page in a new tab</a>
			</div>
			<br/>

			<div>
				<iframe src="https://www.comentarismo.com/store/?showfilter=No&tags=Tooltip&cminds_iframed=1" height="700" style="width: 100%" ></iframe>
			</div>
		...
		public function displayVideoGuidesTab() {
			...
			<div>
				<a href="https://www.comentarismo.com/cm-plugins-video-library/" class="btn button button-primary" target="_blank">Open Video Guides page in a new tab</a>
			</div>
			<br/>

			<div>
				<iframe src="https://www.comentarismo.com/cm-plugins-video-library/?cminds_iframed=1" height="700" style="width: 100%" ></iframe>
			</div>
...
//			}
		...
		public function getUserguideUrl() {
			...
			if ( $this->getOption( 'plugin-userguide-key' ) ) {
				$url .= 'https://www.comentarismo.com/wordpress-plugins-knowledge-base-and-documentation/?hscat=' . $this->getOption( 'plugin-userguide-key' );
			} else {
				$url .= 'https://www.comentarismo.com/wordpress-plugins-support-documentation/';
			}
			return $url;
		}
...
//						'30' => 'Upgrade Tutorial',
		...
		protected function getAds() {
			...
			if ( empty( $ads_arr ) && !is_array( $ads_arr ) ) {
				...
				$href		 = 'https://www.comentarismo.com/wp-admin/admin-ajax.php?action=get_ads&cminds_json_api=get_ads';
				$response	 = wp_remote_post( $href, $args );
				if ( !is_wp_error( $response ) ) {
					$ads = json_decode( wp_remote_retrieve_body( $response ), true );
				} else {
					$args[ 'sslverify' ] = false;
					$href				 = 'http://www.comentarismo.com/wp-admin/admin-ajax.php?action=get_ads&cminds_json_api=get_ads';
					$response			 = wp_remote_post( $href, $args );
					if ( !is_wp_error( $response ) ) {
						$ads = json_decode( wp_remote_retrieve_body( $response ), true );
					} else {
		...
		public function getCategoryLink( $group, $type, $name ) {
			...
			$link = 'https://www.comentarismo.com' . $link;
			return $link;
		}
		...
		public function showRegistration( $atts = array() ) {
			...
			if ( !empty( $post ) && !empty( $post[ 'cminds_nonce' ] ) ) {
				...
				if ( $nonceCheck ) {
					...
					$href		 = 'https://www.comentarismo.com/wp-admin/admin-ajax.php?action=add_user&cminds_json_api=add_user';
					$response	 = wp_remote_post( $href, $args );
					if ( !is_wp_error( $response ) ) {
						...
						$args[ 'sslverify' ] = false;
						$href				 = 'http://www.comentarismo.com/wp-admin/admin-ajax.php?action=add_user&cminds_json_api=add_user';
						$response			 = wp_remote_post( $href, $args );
						if ( !is_wp_error( $response ) ) {
							$result = json_decode( wp_remote_retrieve_body( $response ), true );
							if ( $result && 1 === $result[ 'result' ] ) {
